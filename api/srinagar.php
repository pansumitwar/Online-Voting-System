<?php
     ...
     $check1 = mysqli_query($connect, "SELECT * FROM user WHERE (role=1 OR role=2) AND area=1");
     $userdata1 = mysqli_fetch_all($check1);
     $total1 = count($userdata1);

     $check2 = mysqli_query($connect, "SELECT * FROM user WHERE (role=1 OR role=2) AND area=1 AND status=1");
     $userdata2 = mysqli_fetch_all($check2);
     $total2 = count($userdata2);

     $check3 = mysqli_query($connect, "SELECT * FROM user WHERE (role=1 OR role=2) AND area=1 AND status=0");
...
<html>
    ...
    <meta charset="UTF-8">
        ...
        <title>Srinagar</title>
    </head>
    <body align="center">
        <br><br><br><br><br><h1>AREA : SRINAGAR</h1><br>
